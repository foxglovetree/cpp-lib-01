...
#include "fgt/util/Iterator.h"
#include "fgt/util/Lang.h"
#include "fgt/util/Math.h"
#include "fgt/util/defines.h"
#include "debug.h"

#ifndef fgt_BUFFER_DEBUG
#define fgt_BUFFER_DEBUG (0)
#endif
namespace fgt::util {
...
public:
    template <typename X>
    class Iterator : public fgt::util::Iterator<X> {
    public:
        Buffer<X> *buffer;
        int idx;
...
public:
    ...
    void add(const T *elements, const int from, const int len) {
...
#if fgt_BUFFER_DEBUG == 1
        fgt_DEBUG4(">>Buffer::add,length_: ", this->length_, ",capacity_:", this->capacity_);
#endif
        ...
        Array::add<T>(this->buffer_, this->length_, this->capacity_,
                      ...
                      elements, from, len);
#if fgt_BUFFER_DEBUG == 1
        fgt_DEBUG("<<Buffer<T> *add(const T *elements, const int from, const int len)");
#endif
    }
